...
class User {
	...
	incProgress(rank) {
		if (rank > 8 || rank < -8 || rank === 0) {
			throw new console.error('not valid rank');
		}
		if (this.rank === 8) {
			return;
		}
		if (this.rank === rank) {
			...
			if (this.progress > 100) {
				...
				this.progress = this.progress - Math.floor(this.progress / 100) * 100;
			}
		} else if (this.rank - 1 === rank || (this.rank === 1 && rank === -1)) {
			this.progress += 1;
			if (this.progress > 100) {
				this.rank += Math.floor(this.progress / 100);
				if (this.rank === 0) {
			...
			if ((this.rank > 0 && rank > 0) || (this.rank < 0 && rank < 0)) {
				this.progress += 10 * (rank - this.rank) ** 2;
				...
				this.progress += 10 * (rank - this.rank - 1) ** 2;
